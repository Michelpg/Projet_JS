const getRandomQuiz = (difficulty, themeCode) => {
  return new Promise((resolve, reject) => {
    const pool = require("./connexionBDD");
    const sqlGetRandomQuiz = "SELECT * FROM quiz WHERE difficulte = ? AND theme = ? ORDER BY RAND() LIMIT 1";
    pool.query(sqlGetRandomQuiz, [difficulty, themeCode], function (err, results) {
      if (err) {
        ...
        if (results.length > 0) {
          ...
          reject("Aucun quiz trouvé dans la base de données avec les paramètres spécifiés.");
        }
      }
    });
  });
